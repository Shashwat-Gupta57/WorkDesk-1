...
function GraphInner() {
  ...
  const onNodeDoubleClick: NodeMouseHandler = useCallback((_e, node) => {
    const raw = node.data.rawNode as GraphNode;
    if (raw.type === "artifact") router.push(`/archive/${raw.id}`);
  ...
  if (!graphData || graphData.nodes.length === 0) return (
    <div className="flex h-full flex-col items-center justify-center gap-4">
      <p className="text-sm text-text-secondary">No data to display yet.</p>
      <p className="text-xs text-text-secondary/60">Create sets and artifacts in your Archive to see them here.</p>
      ...
      <div style={{
        ...
        <div style={{ position: "relative" }}>
          ...
          <input
            ...
            style={{
              paddingLeft: 24, paddingRight: search ? 28 : 10, paddingTop: 5, paddingBottom: 5,
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#8b949e", cursor: "pointer", fontSize: 13, lineHeight: 1, padding: 0 }}
            >
              ✕
            </button>
          )}
      ...
      >
        ...
        <MiniMap
          ...
          nodeColor={node => {
            ...
            return colorMap[raw.artifactType ?? ""] ?? "#6e7681";
          }}
          maskColor="#0d111788"
        />
      </ReactFlow>
